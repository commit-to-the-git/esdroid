...
#include <codecvt>
#include <locale>
#include <cstring>  // for strcmp in linkprograms uniform-block name lookup
...
ysError ysOpenGLDevice::CreateRenderingContext(ysRenderingContext **context, ysWindow *window) {
    ...
    *context = nullptr;
#if defined(__ANDROID__)
    ysOpenGLEglContext *newContext = m_renderingContexts.NewGeneric<ysOpenGLEglContext>();
    YDS_NESTED_ERROR_CALL(newContext->CreateRenderingContext(this));
...
ysError ysOpenGLDevice::UpdateRenderingContext(ysRenderingContext *context) {
    ...
    if (window == nullptr) return YDS_ERROR_RETURN(ysError::InvalidParameter);
    const int width = window->GetGameWidth();
    const int height = window->GetGameHeight();
    const int pwidth = window->GetScreenWidth();
    const int pheight = window->GetScreenHeight();
...
ysError ysOpenGLDevice::SetFaceCullingMode(CullMode cullMode) {
    ...
    case CullMode::Back:
        ...
        break;
    case CullMode::None:
        glCullFace(GL_BACK); // gl_none not valid in gles
        break;
    default:
...
ysError ysOpenGLDevice::DestroyGPUBuffer(ysGPUBuffer *&buffer) {
    ...
    if (!CheckCompatibility(buffer))    return YDS_ERROR_RETURN(ysError::IncompatiblePlatforms);                    return YDS_ERROR_RETURN(ysError::InvalidParameter);
...
ysError ysOpenGLDevice::CreateVertexShader(ysShader **newShader, const wchar_t *shaderFilename,
    ...
    *newShader = nullptr;
    ...
    #if defined(__ANDROID__)
    {char _np[512]; size_t _ni=0; for(const wchar_t*_wp=shaderFilename;*_wp&&_ni<511;++_wp,_ni++) _np[_ni]=(char)*_wp; _np[_ni]=0;
    
    esdroid_wtflog("  CreateVertexShader: opening %s", _np);}
#endif
...
#if defined(__ANDROID__)
        esdroid_wtflog("  VERTEX SHADER COMPILE ERROR: %s", errorBuffer);
#endif
...
#if defined(__ANDROID__)
    {
        ...
        m_realContext->glGetShaderInfoLog(handle, 2047, NULL, infoBuffer);
        if (infoBuffer[0] != 0) {
            esdroid_wtflog("  VS info log (%s): %s", shaderName, infoBuffer);
        } else {
            esdroid_wtflog("  VS compiled OK (%s)", shaderName);
        }
    }
#endif
...
ysError ysOpenGLDevice::CreatePixelShader(ysShader **newShader, const wchar_t *shaderFilename,
    ...
    *newShader = nullptr;
...
#if defined(__ANDROID__)
        esdroid_wtflog("  FRAGMENT SHADER COMPILE ERROR: %s", errorBuffer);
#endif
    ...
    {
        char infoBuffer[2048] = {0};
        m_realContext->glGetShaderInfoLog(shaderHandle, 2047, NULL, infoBuffer);
        if (infoBuffer[0] != 0) {
            esdroid_wtflog("  FS info log (%s): %s", shaderName, infoBuffer);
        } else {
            esdroid_wtflog("  FS compiled OK (%s)", shaderName);
        }
    }
#endif
...
ysError ysOpenGLDevice::DestroyShaderProgram(ysShaderProgram *&program, bool destroyShaders) {
    ...
    for (int i = 0; i < (int)ysShader::ShaderType::NumShaderTypes; i++) {
        ...
        if (openglShader != nullptr && m_realContext != nullptr) {
...
ysError ysOpenGLDevice::LinkProgram(ysShaderProgram *program) {
    ...
    if (status == GL_FALSE) {
        ...
        {
            char linkLog[4096] = {0};
            glGetProgramInfoLog(openglProgram->m_handle, 4095, nullptr, linkLog);
            esdroid_wtflog("  PROGRAM LINK ERROR: %s", linkLog);
        }
#endif
...
#if defined(__ANDROID__)
    {
        ...
        glGetProgramInfoLog(openglProgram->m_handle, 4095, nullptr, linkLog);
        if (linkLog[0] != 0) {
            esdroid_wtflog("  Program link info: %s", linkLog);
        } else {
            esdroid_wtflog("  Program linked OK (handle=%u)", openglProgram->m_handle);
        }
    }
    ...
    {
        GLint nAttribs = 0;
        glGetProgramiv(openglProgram->m_handle, GL_ACTIVE_ATTRIBUTES, &nAttribs);
        esdroid_wtflog("  Active attributes: %d", nAttribs);
        for (int i = 0; i < nAttribs; ++i) {
            char name[256] = {0};
            GLint size = 0; GLenum type = 0;
            glGetActiveAttrib(openglProgram->m_handle, (GLuint)i, 255, nullptr, &size, &type, name);
            GLint loc = glGetAttribLocation(openglProgram->m_handle, name);
            esdroid_wtflog("    attr[%d] name='%s' loc=%d size=%d type=0x%x",
                           i, name, loc, size, (unsigned)type);
        }
    }
    // dump active uniforms
    {
        GLint nUniforms = 0;
        glGetProgramiv(openglProgram->m_handle, GL_ACTIVE_UNIFORMS, &nUniforms);
        esdroid_wtflog("  Active uniforms: %d", nUniforms);
    }
    // dump active uniform blocks
    if (m_realContext->glGetActiveUniformBlockName != nullptr) {
        GLint blockCount = 0;
        m_realContext->glGetProgramiv(openglProgram->m_handle,
            GL_ACTIVE_UNIFORM_BLOCKS, &blockCount);
        esdroid_wtflog("  Active uniform blocks: %d", blockCount);
        for (int i = 0; i < blockCount; ++i) {
            char blockName[256] = {0};
            GLsizei nameLen = 0;
            m_realContext->glGetActiveUniformBlockName(
                openglProgram->m_handle, (GLuint)i, 255, &nameLen, blockName);
            // read the binding before we set it
            GLint curBinding = -1;
            m_realContext->glGetActiveUniformsiv(openglProgram->m_handle, 1,
                (const GLuint*)&i, GL_UNIFORM_BLOCK_BINDING, &curBinding);
            esdroid_wtflog("    block[%d] name='%s' (current binding=%d)",
                           i, blockName, curBinding);
        }
    }
#endif

    // glsl es 3.00 has no layoutbinding = n for uniform blocks so every
    // block defaults to binding point 0 remap the known blocks to the slots
    // the c++ side uses after linking
    if (m_realContext->glUniformBlockBinding != nullptr &&
        m_realContext->glGetActiveUniformBlockName != nullptr) {
        GLint blockCount = 0;
        m_realContext->glGetProgramiv(openglProgram->m_handle,
            GL_ACTIVE_UNIFORM_BLOCKS, &blockCount);
        ...
        for (GLint i = 0; i < blockCount; ++i) {
            ...
            GLsizei nameLen = 0;
            m_realContext->glGetActiveUniformBlockName(
                openglProgram->m_handle,
                (GLuint)i,
                (GLsizei)(sizeof(blockName) - 1),
                &nameLen,
                blockName);

            if (nameLen <= 0) continue;

            GLuint bindingPoint = 0xFFFFFFFFu;  // sentinel unknown block
            if (strcmp(blockName, "ScreenVariables") == 0) {
                bindingPoint = 0;  // matches c++ scenedata slot
            } else if (strcmp(blockName, "ObjectVariables") == 0) {
                bindingPoint = 1;  // matches c++ objectdata slot
            } else if (strcmp(blockName, "SkinningVariables") == 0) {
                bindingPoint = 2;  // matches c++ skinning slot unused on android
            } else if (strcmp(blockName, "Lighting") == 0) {
                bindingPoint = 3;  // matches c++ lighting slot
            }

            if (bindingPoint != 0xFFFFFFFFu) {
                m_realContext->glUniformBlockBinding(
                    openglProgram->m_handle, (GLuint)i, bindingPoint);
            }
        }
    }

    // samplers also default to texture unit 0 in glsl es 3.00 assign the
    // known sampler uniforms to their texture units gluniform1i needs the
    // program active so bind it temporarily and restore the previous one
    if (m_realContext->glGetUniformLocation != nullptr &&
        m_realContext->glUniform1i != nullptr &&
        m_realContext->glUseProgram != nullptr) {
        GLint prevProgram = 0;
        glGetIntegerv(GL_CURRENT_PROGRAM, &prevProgram);
        m_realContext->glUseProgram(openglProgram->m_handle);

        struct SamplerBind { const char *name; GLint unit; };
        static const SamplerBind knownSamplers[] = {
            { "diffuseTex", 0 },
            { "layer0",     0 },
            { "layer1",     1 },
        };
        for (const auto &s : knownSamplers) {
            GLint loc = m_realContext->glGetUniformLocation(
                openglProgram->m_handle, s.name);
            if (loc >= 0) {
                m_realContext->glUniform1i(loc, s.unit);
            }
        }

        m_realContext->glUseProgram((GLuint)prevProgram);
    }
...
void ysOpenGLDevice::Draw(int numFaces, int indexOffset, int vertexOffset) {
    if (m_activeVertexBuffer != nullptr) {
        if (m_realContext->glDrawElementsBaseVertex != nullptr) {
            m_realContext->glDrawElementsBaseVertex(GL_TRIANGLES, numFaces * 3, GL_UNSIGNED_SHORT, (void *)(indexOffset * 2), vertexOffset);
        } else {
            // gldrawelementsbasevertex is not available gles 3.0 without the
            // gl_oes_draw_elements_base_vertex extension fall back to
            // gldrawelements which does not apply vertexoffset the geometry
            // generator and model loader write absolute indices so
            // vertexoffset is always 0 here
    ...
    ysRenderTarget::Format format, bool colorData, bool depthBuffer) 
{
    ...
    if (!colorData) {
        // gltexparameterigl_texture_2d gl_texture_compare_mode gl_compare_ref_to_texture
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
