...
dbasic::DeltaEngine::CreateGameWindow(const GameEngineSettings &settings) {
    ...
    #if defined(__ANDROID__)
    {std::wstring _wt; for(const char *_p=settings.WindowTitle;*_p;++_p) _wt.push_back((wchar_t)(unsigned char)*_p);
    YDS_NESTED_ERROR_CALL(m_gameWindow->InitializeWindow(
            nullptr, _wt.c_str(), settings.WindowStyle,
    ...
    YDS_NESTED_ERROR_CALL(ysInputSystem::CreateInputSystem(
            &m_inputSystem, ysWindowSystemObject::Platform::Android))
#else
    YDS_NESTED_ERROR_CALL(ysInputSystem::CreateInputSystem(
    ...
    for(const char *_p=settings.ShaderCompiledDirectory;*_p;++_p) _wscd.push_back((wchar_t)(unsigned char)*_p);
    YDS_NESTED_ERROR_CALL(InitializeShaders(_wsd.c_str(), _wscd.c_str(), settings.CompileShaders));}
#else
    ...
    #if defined(__ANDROID__)
    {std::wstring _wld; for(const char *_p=settings.LoggingDirectory;*_p;++_p) _wld.push_back((wchar_t)(unsigned char)*_p); InitializeBreakdownTimer(_wld.c_str());}
#else
...
ysError dbasic::DeltaEngine::StartFrame() {
    ...
    if (m_timingSystem != nullptr) {
        m_timingSystem->Update();
    } else {
        m_timingSystem = ysTimingSystem::Get();
        if (m_timingSystem) m_timingSystem->Initialize();
    }
...
// teardown helper for destroy a failed step is remembered but never
// aborts the sequence the old nested error macros returned early and a
// single null object the skinned shader objects are never created on
// android used to abort the whole chain before the window system was
// destroyed the window system singleton then blocked every later
// creategamewindow call with multipleerrorsystems
#define DELTA_DESTROY_STEP(expr)                                               \
    {                                                                          \
        const ysError stepError = (expr);                                      \
        if (stepError != ysError::None && destroyError == ysError::None) {     \
            destroyError = stepError;                                          \
        }                                                                      \
    }
...
ysError dbasic::DeltaEngine::Destroy() {
    ...
    ysError destroyError = ysError::None;

    DELTA_DESTROY_STEP(GetConsole()->Destroy());
    DELTA_DESTROY_STEP(GetUiRenderer()->Destroy());

    DELTA_DESTROY_STEP(m_device->DestroyGPUBuffer(m_mainIndexBuffer));
    DELTA_DESTROY_STEP(m_device->DestroyGPUBuffer(m_mainVertexBuffer));
    DELTA_DESTROY_STEP(
            m_device->DestroyGPUBuffer(m_consoleShaderObjectVariablesBuffer));

    DELTA_DESTROY_STEP(m_device->DestroyShader(m_vertexShader));
    if (m_vertexSkinnedShader != nullptr) {
        DELTA_DESTROY_STEP(m_device->DestroyShader(m_vertexSkinnedShader));
    }
    DELTA_DESTROY_STEP(m_device->DestroyShader(m_consoleVertexShader));
    DELTA_DESTROY_STEP(m_device->DestroyShader(m_saqVertexShader));

    DELTA_DESTROY_STEP(m_device->DestroyShader(m_pixelShader));
    DELTA_DESTROY_STEP(m_device->DestroyShader(m_saqPixelShader));
    DELTA_DESTROY_STEP(m_device->DestroyShader(m_consolePixelShader));

    DELTA_DESTROY_STEP(m_device->DestroyShaderProgram(m_shaderProgram));
    DELTA_DESTROY_STEP(m_device->DestroyShaderProgram(m_consoleProgram));
    if (m_skinnedShaderProgram != nullptr) {
        DELTA_DESTROY_STEP(
                m_device->DestroyShaderProgram(m_skinnedShaderProgram));
    }

    DELTA_DESTROY_STEP(m_device->DestroyInputLayout(m_inputLayout));
    if (m_skinnedInputLayout != nullptr) {
        DELTA_DESTROY_STEP(m_device->DestroyInputLayout(m_skinnedInputLayout));
    }
    DELTA_DESTROY_STEP(m_device->DestroyInputLayout(m_consoleInputLayout));
    DELTA_DESTROY_STEP(m_device->DestroyInputLayout(m_saqInputLayout));

    DELTA_DESTROY_STEP(m_device->DestroyRenderTarget(m_mainRenderTarget));
    DELTA_DESTROY_STEP(m_device->DestroyRenderingContext(m_renderingContext));

    DELTA_DESTROY_STEP(m_device->DestroyDevice());
    ...
    DELTA_DESTROY_STEP(ysInputSystem::DestroyInputSystem(m_inputSystem));

    if (destroyError == ysError::None) {
        return YDS_ERROR_RETURN(ysError::None);
    }
    return destroyError;
}

#undef DELTA_DESTROY_STEP
...
float dbasic::DeltaEngine::GetFrameLength() {
    return m_timingSystem ? (float)(m_timingSystem->GetFrameDuration()) : 1.0f/60.0f;
}

float dbasic::DeltaEngine::GetAverageFramerate() {
    return m_timingSystem ? m_timingSystem->GetFPS() : 60.0f;
}
...
void dbasic::DeltaEngine::SetPaused(bool paused) {
    if (paused) {
        ...
        if (m_timingSystem) m_timingSystem->RestartFrame();
    }
}
