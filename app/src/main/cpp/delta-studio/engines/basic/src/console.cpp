...
ysError dbasic::Console::Initialize() {
    ...
    std::wstring fontPathW = m_defaultFontDirectory + L"Silkscreen/slkscr.ttf";
    std::string fontPathNarrow(fontPathW.begin(), fontPathW.end());
#if defined(__ANDROID__)
    esdroid_wtflog("Console::Initialize: loading font from %s", fontPathNarrow.c_str());
#endif
    ysError fontErr = m_engine->LoadFont(
            &m_font,
            fontPathW.c_str());
#if defined(__ANDROID__)
    esdroid_wtflog("Console::Initialize: LoadFont returned %d, m_font=%p", (int)fontErr, (void*)m_font);
    if (m_font != nullptr) {
        esdroid_wtflog("Console::Initialize: font loaded OK, fontHeight=%f", m_font->GetFontHeight());
    } else {
        esdroid_wtflog("Console::Initialize: FONT FAILED TO LOAD, text rendering will be skipped");
    }
#endif
