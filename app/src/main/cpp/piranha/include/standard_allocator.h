...
namespace piranha {
    ...
    public:
        ...
        t_alloc *allocate(unsigned int n = 1, unsigned int alignment = 1) {
            ...
            t_alloc *newObject;
#if defined(__ANDROID__)
                if (sizeof(t_alloc) * n > 1000000) esdroid_wtflog("LARGE ALLOC: type=%s size=%d", typeid(t_alloc).name(), (int)(sizeof(t_alloc) * n));
#endif
            ...
            if (alignment == 1) {
                ...
                void *buffer = nullptr;
                if (posix_memalign(&buffer, alignment, sizeof(t_alloc) * n) != 0) buffer = nullptr;
                if (!buffer) { esdroid_wtflog("ALLOC FAILED: size=%d align=%d", (int)(sizeof(t_alloc) * n), alignment); }
#else
