...
piranha::IrCompilationUnit *piranha::Compiler::analyze(const IrPath &scriptPath) {
    IrPath canonPath = scriptPath.canonicalize();
    IrCompilationUnit *newUnit = getUnit(canonPath);
    Path rootDir;
    ...
    if (newUnit == nullptr) {
        ...
        IrCompilationUnit::ParseResult parseResult = newUnit->parseFile(canonPath);
