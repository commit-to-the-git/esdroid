...
void CylinderHead::initialize(const Parameters &params) {
    ...
    esdroid_wtflog("CylinderHead::initialize: cylinderCount=%d", params.Bank->getCylinderCount());
#endif
    m_cylinders = new Cylinder[params.Bank->getCylinderCount()];
#if defined(__ANDROID__)
    esdroid_wtflog("CylinderHead: allocated %d cylinders", params.Bank->getCylinderCount());
    esdroid_wtflog("CylinderHead: valvetrain=%p", (void*)params.Valvetrain);
#endif
