...
dbasic::Vertex *GeometryGenerator::writeVertex() {
    if (m_state.vertexPointer >= m_vertexBufferSize) {
        return &m_vertexData[m_vertexBufferSize - 1];
    }
...
void GeometryGenerator::writeFace(unsigned short i0, unsigned short i1, unsigned short i2) {
    if (m_state.indexPointer + 2 < m_indexBufferSize) {
        ...
        m_indexData[m_state.indexPointer + 2] = i2 + m_state.subshapeVertexPointer + base;
    }
