...
void OscilloscopeCluster::nextFocusScope() {
    ...
    Oscilloscope *groups[GroupCount][MaxLayeredScopes] = {
        { m_audioWaveformScope, nullptr },
        { m_torqueScope, m_powerScope, nullptr },
        { m_totalExhaustFlowScope, nullptr },
        { m_intakeValveLiftScope, m_exhaustValveLiftScope, nullptr },
        { m_pvScope, nullptr },
        { m_intakeFlowScope, m_exhaustFlowScope, m_cylinderMoleculesScope, nullptr },
        { m_cylinderPressureScope, nullptr },
        { m_sparkAdvanceScope, nullptr }
    };
    ...
    for (int i = 0; i < GroupCount; ++i) {
        if (groups[i][0] == m_currentFocusScopes[0]) {
            current = i;
            break;
        }
    }

    const int next = (current + 1) % GroupCount;
    for (int i = 0; i < MaxLayeredScopes; ++i) {
        m_currentFocusScopes[i] = groups[next][i];
    }
}
