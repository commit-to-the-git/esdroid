...
// All log lines go through one append mode fd. Each line is a single write()
// call so lines from different threads never interleave, and the crash
// handler can write without touching stdio locks or the heap.
...
static void initCrashLogger() {
    ...
    // Delete the previous run's log, then start a fresh one.
    // piranha_errors.log is rewritten by every script compile.
    // java_crash.log and last_logcat.txt belong to the Java side and are
    // managed there, they are deliberately not touched here.
...
// A clean process exit passes through here. A death with no crash line and
// no exit line means the process was killed outright (SIGKILL, ANR kill,
// low memory), which no in process handler can record.
...
static EngineSimApplication* createApplication(android_app* app) {
    ...
    try {
        ...
        // initialize() returns early when a core system fails, for example
        // CreateGameWindow. Such an application has no window and no device,
        // running it would crash, and destroy() skips its teardown. Return
        // null so the caller never touches it.
...
extern "C" void android_main(struct android_app* app) {
    ...
    // The native_app_glue thread now has 16MB stack (see android_native_app_glue.c).
    // We run directly on this thread so the looper processes window/input events.
    ...
    // Only extract if the assets directory doesn't exist or is empty.
    // This avoids re-extracting (and re-parsing/recompiling) on every launch.
    ...
    // Remove an engine imported in a previous session. Every launch starts on
    // the default engine and imports are not kept in storage. The legacy
    // files/imported.mr location is cleaned too so an upgrade from an older
    ...
    do {
        ...
        // Initialize touch UI overlay (drawn on top of the engine)
        ...
        if (restart && !esdroid::AndroidBackend::instance().shouldQuit()) {
            // Record the imported file size so a bad or empty copy is visible
            ...
            // The picker may still be closing. Wait for the window to come
            // back so teardown and rebuild run with a live GL context.
        ...
        // The engine teardown releases its GL objects on the shared EGL
        // context. If anything took the surface down with it, bring it back
