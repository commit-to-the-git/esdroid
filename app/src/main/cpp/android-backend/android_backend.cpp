...
static int32_t handle_input_event(android_app* app, AInputEvent* event) {
    ...
    if(type==AINPUT_EVENT_TYPE_MOTION) {
        ...
        case AMOTION_EVENT_ACTION_CANCEL: {
            ...
            for(int i=0;i<pc;++i) b.releaseAllButtons(AMotionEvent_getPointerId(event,i));
            return 1;
        }
        }
    }
...
bool AndroidBackend::pollEvents() {
    ...
    // Input events are handled by the native_app_glue's internal looper
    // callback (app->onInputEvent) which we set in esdroid_install_app_callbacks.
    // The looper processes input events when source->process() is called above,
    // which calls android_app_input() -> app->onInputEvent -> handle_input_event.
    // Do NOT process input events here, they are already handled.
    return !m_shouldQuit;
}
...
void AndroidBackend::pressButton(TouchButton* btn,int pid) {
    ...
    if(btn->key==VirtualKey::Insert) requestMrFilePicker();
}
...
void AndroidBackend::releaseButton(TouchButton* btn) {
    ...
    int k=(int)btn->key;
    if(k>0&&k<(int)VirtualKey::Count) m_keyState[k]=false;
}

void AndroidBackend::releaseAllButtons(int pid) {
    for(auto& b:m_buttons) if(b.pointerId==pid) releaseButton(&b);
}
...
void AndroidBackend::layoutButtons(int sw,int sh) {
    ...
    auto add=[&](const char* l,float x,float y,VirtualKey k){
        m_buttons.push_back({l,x,y,bw,bh,k,false,false,-1});
    };
    float lY=sh-pad-bh;
    add("STARTER",pad,lY,VirtualKey::Starter);
    add("IGNITION",pad,lY-(bh+gap),VirtualKey::Ignition);
    add("THROTTLE",pad,lY-2*(bh+gap),VirtualKey::Throttle);
    add("CLUTCH",pad,lY-3*(bh+gap),VirtualKey::Clutch);
    float rX=sw-pad-bw;
    add("SHIFT +",rX,lY,VirtualKey::ShiftUp);
    add("SHIFT -",rX,lY-(bh+gap),VirtualKey::ShiftDown);
    add("PAUSE",rX,lY-2*(bh+gap),VirtualKey::Pause);
    // ResetEngine maps to ysKey::Code::Return in the shim, which is the
    // Enter key: run() reloads the engine script on its edge (loadScript()).
    add("RELOAD",rX,lY-3*(bh+gap),VirtualKey::ResetEngine);
    add("1x",pad,pad,VirtualKey::Return);
    add("1/10x",pad,pad+(bh+gap),VirtualKey::N1);
    add("1/100x",pad,pad+2*(bh+gap),VirtualKey::N2);
    add("IMPORT",rX,pad,VirtualKey::Insert);
    add("EXIT",rX,pad+(bh+gap),VirtualKey::Escape);
    add("CAMERA",rX,pad+2*(bh+gap),VirtualKey::Camera);
    // No desktop key pages the oscilloscope focus (Right only steps one
    // frame while paused and touch never reaches the UI elements), so this
    // button is handled directly in EngineSimApplication::run().
    add("OSC PAGE",rX,pad+3*(bh+gap),VirtualKey::OscPage);
}
...
static std::string buildImportedMainScript(const std::string& enginePath) {
    ...
    // Strip a .mr suffix so the import line can re-add it cleanly
...
// Java VM pointer for JNI callbacks
...
extern "C" void esdroid_swap_buffers() {
    // Render touch UI overlay on top of the engine before swapping
