...
public:
    ...
    virtual ~ysAndroidInputDevice() {
        ...
        // Just null our derived pointers to avoid double-free
        m_androidKb = nullptr;
...
bool ysAndroidKeyboard::IsKeyDown(ysKey::Code key) {
    for(const auto& b : esdroid::AndroidBackend::instance().buttons()) {
        int mapped=vkToYsKey(b.key);
        // 0 means the button has no engine key (OSC PAGE), and 0 is also
        // ysKey::Code::Back, so an unmapped button must never match.
        if(mapped!=0&&mapped==(int)key&&b.held) return true;
    }
    return false;
}
bool ysAndroidKeyboard::ProcessKeyTransition(ysKey::Code key, ysKey::State state) {
    ...
    for(auto& b : esdroid::AndroidBackend::instance().buttons()) {
        int mapped=vkToYsKey(b.key);
        if(mapped!=0&&mapped==(int)key&&b.edge) { b.edge=false; return true; }
    }
    return false;
}
